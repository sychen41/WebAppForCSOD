...
//SHOW route (By parent_id)
router.get('/parentId/:parent_id', function(req, res){
    var connection = mysql.createConnection(connectionStr);
    connection.connect();
    connection.query('select * from Student_Tab where ParentKey=?', req.params.parent_id, function(err, rows){
        if (err) throw err;
        console.log('SUCCESS: retrieve all students (children by parent Id) info from db');
        res.json(rows); 
    });
    connection.end();
});
